...
from JHUanalyzer.Analyzer.analyzer import analyzer, CutGroup, VarGroup
...
preselected.Snapshot(["SR.*","AT.*","mh","mreduced","Pdfweight","mhh","nFatJet","FatJet_pt"],'snapshot_example.root',treename='preselected')
...
SRTT.DataFrame.Histo2D(("SRTT_pdfUp","SRTT_pdfUp",9 ,40 ,220 ,28 ,700 ,3500),'mh','mhh','Pdfweight_up'),
...
SRTT_cutflow = CutflowHist('cutflow',SRTT.DataFrame,SRTT_cuts) # SRTT.DataFrame already has the cuts and numbers, SRTT_cuts is just to name the histogram bins (but that means they must match up!)
SRTT_cutflow.Write()
